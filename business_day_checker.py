...
# Checks if the current date (today) is the fourth business day of the current month
...
# Variable assignments - takes today's date and forms a date range of currently ongoing month
...
# Holidays library can handle Finnish (and other, if needed) holidays - the library is used to form an array of Finnish holidays
fin_holidays = []
for date in holidays.Finland(years=current_year).items():
    fin_holidays.append(str(date[0]))

# Forms the current month's calendar, excluding the weekends and holidays - i.e., the calendar formed here only contains business days of the month
...
# Compares today's date to current month's fourth business date - if it is, the needed script is executed, otherwise the script exits
